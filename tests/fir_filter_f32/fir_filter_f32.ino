/*
This one works.
  I have created a teensy/libraries/math_helper directory which
  contains math_helper.h and math_helper.c renamed as math_helper.cpp
*/
...
extern float testInput_f32_1kHz_15kHz[TEST_LENGTH_SAMPLES];
extern float refOutput[TEST_LENGTH_SAMPLES];
...
void setup(void)
{
  ...
  while(!Serial);
  delay(200);
  Serial.println("FIR Filter Test");
  ...
  arm_fir_init_f32(&S, NUM_TAPS, (float *)&firCoeffs32[0], &firStateF32[0], blockSize);
  ...
  ** ------------------------------------------------------------------- */
  ...
  snr = arm_snr_f32(&refOutput[0], &testOutput[0], (uint32_t)TEST_LENGTH_SAMPLES);
  Serial.print("SNR = ");
  Serial.println(snr);
  if (snr < SNR_THRESHOLD_F32) {
    Serial.println("failed SNR test");
  } else {
    Serial.println("passed SNR test");
  }
}
