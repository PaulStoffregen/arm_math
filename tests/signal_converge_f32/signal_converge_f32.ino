 ...
 * test_signal_converge() implements the stopping condition. The function checks if all of
 * the values of the error signal have a magnitude below a threshold DELTA.
 *
 * \par Block Diagram:
 * \par
...
float firStateF32[NUMTAPS + BLOCKSIZE];
...
float lmsStateF32[NUMTAPS + BLOCKSIZE];
float errOutput[TEST_LENGTH_SAMPLES];
...
* Internal functions
* ------------------------------------------------------------------- */
arm_status test_signal_converge(float* err_signal, uint32_t blockSize);
void getinput(float* input, uint32_t fr_cnt,  uint32_t blockSize);
...
float wire1[BLOCKSIZE];
float wire2[BLOCKSIZE];
float wire3[BLOCKSIZE];
float err_signal[BLOCKSIZE];
...
void setup(void)
{
  ...
  while(!Serial);
  delay(200);
  Serial.println("Test convergence of an adaptive filter");
  ...
  arm_fir_init_f32(&LPF_instance, NUMTAPS, (float *)FIRCoeff_f32, firStateF32, BLOCKSIZE);
  ...
  * ------------------------------------------------------------------- */
  ...
  arm_sub_f32((float *)FIRCoeff_f32, lmsNormCoeff_f32, lmsNormCoeff_f32, NUMTAPS);
