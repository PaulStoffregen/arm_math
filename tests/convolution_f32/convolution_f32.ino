...
* Declare I/O buffers
* ------------------------------------------------------------------- */
float Ak[MAX_BLOCKSIZE];       /* Input A */
float Bk[MAX_BLOCKSIZE];       /* Input B */
float AxB[MAX_BLOCKSIZE * 2];  /* Output */

/* ----------------------------------------------------------------------
* Test input data for Floating point Convolution example for 32-blockSize
* Generated by the MATLAB randn() function
* ------------------------------------------------------------------- */
float testInputA_f32[64] =
{
...
const float testRefOutput_f32[128] =
{
...
void setup()
{
  ...
  while(!Serial);
  delay(200);
  Serial.println("Test Complex FFT, Complex-by-Complex Multiplication");
  ...
  status = arm_cfft_radix4_init_f32(&cfft_instance, 64, 0, 1);
  if (status != ARM_MATH_SUCCESS) {
    Serial.println("Fail CFFT initialization");
  }

  /* Transform input a[n] from time domain to frequency domain A[k] */
  arm_cfft_radix4_f32(&cfft_instance, Ak);
  /* Transform input b[n] from time domain to frequency domain B[k] */
  arm_cfft_radix4_f32(&cfft_instance, Bk);
  ...
  status = arm_cfft_radix4_init_f32(&cfft_instance, 64, 1, 1);
  if (status != ARM_MATH_SUCCESS) {
    Serial.println("Fail Inverse CFFT");
  }

  /* Transform the multiplication output from frequency domain to time domain,
     that gives the convolved output  */
  arm_cfft_radix4_f32(&cfft_instance, AxB);

  /* SNR Calculation */
  snr = arm_snr_f32((float *)testRefOutput_f32, AxB, srcALen + srcBLen - 1);
